...
export class OffDayRequestService {
    ...
    async updateOffDayRequest(requestId: string, dto: UpdateTimeOffRequestDto, userId: string) {
        const { startDate, endDate, reason, isFullDayOff, totalDaysOff } = dto;
        ...
        return successResponse(updatedRequest, 'Time off request updated successfully');
    }
    ...
    async deleteOffDayRequest(userId: string, requestId: string) {
        ...
        if (!existingRequest || existingRequest.userId !== userId) {
            throw new Error('Request not found or you do not have permission to delete it');
        }
