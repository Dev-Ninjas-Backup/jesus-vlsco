...
export class SurveyResponseController {
  ...
  @ApiOperation({ summary: 'Get all user responses combined as analysis for a specific survey' })
  @Get(':surveyId')
  async getASurveyResponseByAllEmployees(@Param('surveyId') responseId: string) {
    // This method should return all responses for a specific survey
    return this.surveyResponseService.getASurveyResponseByAllEmployees(responseId);
  }

  @ApiOperation({ summary: 'Get all recent survey responses for all employees' })
  @Get('recent')
  ...
  async getAllRecentQuestionsResponsesByAllUsers(
    ...
    return this.surveyResponseService.getAllRecentQuestionsResponsesByAllUsers(pg);
  }
}
